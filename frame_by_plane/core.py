...
def fbp_remove_plane_datablock(plane):
    """Remove a linked FBP plane plus its private mesh/materials. Never deletes image files from disk."""
    if not plane:
        return False
    try:
...
def fbp_repair_default_duplicates(context):
...
def cleanup_orphan_fbp_planes_timer():
    if bpy.context:
        fbp_cleanup_planes_for_deleted_rigs(bpy.context)
        fbp_repair_default_duplicates(bpy.context)
        cleanup_orphan_fbp_planes(bpy.context, force=True)
        sync_layer_collection(bpy.context)
        fbp_snapshot_layer_plane_links(bpy.context)
    return 0.75
...
def fbp_rebuild_original_materials_for_holdout_restore(rig):
    """Best-effort fallback for old scenes where holdout overwrote its saved originals."""
    plane = getattr(rig, "fbp_plane_target", None)
    if not plane or not getattr(plane, "data", None):
        return False

    # Procedural color/gradient planes can rebuild their editable material from rig settings.
    if getattr(rig, "fbp_is_color_plane", False):
    ...
    # Image planes can rebuild their sequence materials from the stored frame filepaths.
    ...
    for idx, item in enumerate(rig.fbp_images):
        if getattr(item, "is_empty", False) or not getattr(item, "filepath", ""):
            plane.data.materials.append(create_fbp_empty_material(f"FBP_Empty_Frame_{rig.name}_{idx + 1}"))
            continue
        mat_name = f"Mat_{getattr(item, 'name', '') or os.path.basename(getattr(item, 'filepath', ''))}"
        plane.data.materials.append(create_fbp_material(
            mat_name,
            getattr(item, "filepath", ""),
            interp=interp,
            opacity=opacity,
    ...
    try:
        ...
        pass
    return bool(plane.data.materials)
...
class FBP_OT_ImportSingleImage(Operator):
    ...
    bl_description = "Create one Frame by Plane layer from one image, or one animated plane from multiple selected images"
    bl_options = {'REGISTER', 'UNDO'}
...
def draw_fbp_delete_menu(self, context):
    if get_selected_fbp_roots(context):
        self.layout.separator()
        self.layout.operator("fbp.delete_sequence", text="Frame by Plane: Delete Rig + Image Plane", icon=fbp_icon("TRASH"))
...
def register_fbp_menus():
    ...
    for menu_name in ("VIEW3D_MT_object_delete", "OUTLINER_MT_object_delete"):
        delete_menu = getattr(bpy.types, menu_name, None)
        if delete_menu:
            delete_menu.prepend(draw_fbp_delete_menu)
...
def unregister_fbp_menus():
    ...
    for menu_name in ("VIEW3D_MT_object_delete", "OUTLINER_MT_object_delete"):
        ...
        if delete_menu:
            try:
                delete_menu.remove(draw_fbp_delete_menu)
            except Exception:
                pass
...
def register():
    ...
    if bpy.context and not bpy.app.timers.is_registered(cleanup_orphan_fbp_planes_timer):
        bpy.app.timers.register(cleanup_orphan_fbp_planes_timer, first_interval=0.75)
