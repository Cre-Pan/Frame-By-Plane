...
def fbp_cancel_deferred_mutation_tasks():
    ...
    try:
        ...
        return int(_fbp_safe_tasks.clear_scheduled() or 0)
...
def fbp_deferred_post_undo_sync():
    ...
    try:
        ...
        if context and getattr(context, "scene", None):
            ...
            # Persist geometry-effect instance identities on the owning rig.
            # Blender 5.1 does not reliably serialize custom properties stored
            # only on modifiers, so files created before 5.5.18 are normalized
            # once after load/scene switch without rebuilding their effects.
            try:
                from .geometry_nodes import fbp_refresh_effect_instance_ids
                for rig in context.scene.objects:
                    if bool(getattr(rig, "is_fbp_control", False)):
                        fbp_refresh_effect_instance_ids(rig)
            except (ImportError, AttributeError, ReferenceError, RuntimeError, TypeError, ValueError):
                pass
            try:
...
def fbp_undo_pre_handler(scene):
    ...
    # Never let a timer captured before Ctrl+Z resume against restored datablocks.
    # Essential UI/native repair work is scheduled again by post-Undo sync.
...
def fbp_load_post_handler(_dummy):
    ...
    # Blender 5.1 may not persist custom properties stored only on Geometry
    # Nodes modifiers. Normalize the mirrored rig-owned effect identities while
    # load_post owns a stable new Main, before diagnostics or UI reads occur.
    try:
        from .geometry_nodes import fbp_refresh_effect_instance_ids
