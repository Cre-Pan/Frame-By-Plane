...
def _fbp_scene_snapshot_key(scene):
    ...
    Blender ``session_uid`` values are already unique for the loaded Main and
    therefore survive Scene renames. Only the legacy pointer/name fallback needs
    the extra name guard against pointer reuse.
    """
...
def _fbp_known_links_store_entry(store, scene):
    """Resolve current snapshots and transparently read the 5.3.7 key format."""
    ...
    if isinstance(entry, dict):
        return scene_key, entry

    # 5.3.7 keyed the store only by the runtime integer and retained ``scene_ref``.
    # Read that entry once so an open development session upgrades cleanly.
    legacy_key = _fbp_scene_runtime_identity(scene)
    legacy_entry = store.get(legacy_key) if legacy_key is not None else None
    if isinstance(legacy_entry, dict):
        legacy_scene = legacy_entry.get("scene_ref")
        if legacy_scene is not None and not fbp_obj_matches_runtime_key(
            legacy_scene, legacy_key
        ):
            return scene_key, None
        return scene_key, legacy_entry
    return scene_key, None
...
def _fbp_resolve_snapshot_rig(scene, rig_name, identities=None):
    ...
    Old snapshots without an identity keep the historical name fallback. Current
    snapshots require the stored runtime identity and can also recover a direct
    rig rename when the plane temporarily lost its parent pointer.
    """
    name = str(rig_name or "")
...
def _fbp_set_known_links(scene, links, identities=None):
    ...
    # Keep only current Scene identities and migrate away from the 5.3.7
    # integer-only keys. No Blender RNA object is retained in this cache.
    live_scene_keys = set()
    try:
        live_scene_keys = {
            key for item in bpy.data.scenes
...
def _fbp_clear_legacy_layer_parenting(rig):
    """Remove the retired layer-to-layer parenting relationship safely."""
    try:
        parent = getattr(rig, "parent", None)
        if not parent or not is_fbp_layer_object(parent):
            return False
...
def sync_layer_collection(context):
    ...
    for item in sc.fbp_layers:
        try:
            ...
            if not rig or not object_in_scene(rig, sc):
                continue
            _fbp_clear_legacy_layer_parenting(rig)
    ...
    for obj in sc.objects:
        ...
        if is_fbp_layer_object(obj) and key not in existing_keys:
            _fbp_clear_legacy_layer_parenting(obj)
...
def fbp_cleanup_planes_for_deleted_rigs(context):
    ...
    for rig_name, plane_names in list(links.items()):
        ...
        for plane in candidate_planes:
            ...
            if used_elsewhere:
                continue
            removed += 1 if fbp_remove_plane_datablock(plane, pending_materials=pending_materials) else 0
        links.pop(rig_name, None)
        identities.pop(rig_name, None)
...
def fbp_depsgraph_native_ops_handler(scene, depsgraph):
    ...
    try:
        ...
        try:
            ...
            schedule_object_mask_geometry_updates(scene, depsgraph)
        except (ImportError, AttributeError, ReferenceError, RuntimeError, TypeError, ValueError):
            pass
        try:
