...
def fbp_forget_drawing_rig_runtime(rig):
    """Drop runtime-only Cutout state before a rig is deleted or replaced."""
    if not rig:
        return
    key = _drawing_key(rig)
    _DRAWING_SYNC_STATES.pop(key, None)
    _DRAWING_KEY_DEADLINES.pop(key, None)
    _DRAWING_KEY_REQUESTS.pop(key, None)
    # Scene membership changed; rebuild the short name cache on demand.
    _DRAWING_SCENE_RIG_CACHE.clear()
...
def fbp_sync_drawing_texture_settings(rig):
    """Update Cutout filtering/extension without rebuilding its material."""
...
def _is_compatible_drawing_image(image, normalized_path=""):
    """Return True only for ordinary FILE images owned safely by Cutout.

    Native Image Sequence and Movie datablocks can share the same filepath as a
    Cutout drawing. Reusing them couples two different cache/timing contracts;
    changing or freeing the Cutout buffer can then turn animated planes magenta.
    """
    if image is None:
        return False
    try:
        if str(getattr(image, "source", "") or "").upper() != "FILE":
            return False
        if bool(image.get("fbp_native_media_key", "")):
            return False
        if normalized_path:
            return _normalized_image_path(getattr(image, "filepath", "")) == normalized_path
        return True
    except FBP_DATA_ERRORS:
        return False
    ...
    for image in getattr(bpy.data, "images", ()):
        try:
            ...
            if (
                key
                and image_name
                and key not in _DRAWING_IMAGE_PATH_CACHE
                and _is_compatible_drawing_image(image, key)
            ):
                _DRAWING_IMAGE_PATH_CACHE[key] = image_name
        except FBP_DATA_ERRORS:
            continue
...
def _loaded_image_for_path(image_path):
    ...
    # Exact-key hits are far more common than cold lookups during Cutout
    # playback. Do not rescan every Blender Image merely because an unrelated
    # datablock changed elsewhere in the file.
    image_name = str(_DRAWING_IMAGE_PATH_CACHE.get(key, "") or "")
    image = bpy.data.images.get(image_name) if image_name else None
    try:
        if _is_compatible_drawing_image(image, key):
            return image
    except FBP_DATA_ERRORS:
        pass
    _DRAWING_IMAGE_PATH_CACHE.pop(key, None)

    # On a cache miss, index existing IDs once so opening an existing .blend can
    # reuse already loaded drawings without creating duplicate Image datablocks.
    _ensure_loaded_image_path_index()
    image_name = str(_DRAWING_IMAGE_PATH_CACHE.get(key, "") or "")
    image = bpy.data.images.get(image_name) if image_name else None
    try:
    ...
    for candidate in getattr(bpy.data, "images", ()):
        try:
            if _is_compatible_drawing_image(candidate, key):
...
def _drawing_buffer_usage_snapshot(*, include_library=False):
    """Collect Cutout buffer usage in one scene pass.

    Buffer trimming previously scanned every Scene/Cutout rig twice: once for
    active texture images and once for library users. Keeping both results in a
    single snapshot halves the global traversal on large projects.
    """
    active = set()
    expected_users = {}
    candidates = {}
    try:
        for scene in bpy.data.scenes:
            for rig in _drawing_rigs_for_scene(scene):
                ...
                texture_name = (
                    str(getattr(texture_image, "name", "") or "")
                    if texture_image else ""
                )
                if texture_name:
                    active.add(texture_name)
                    if include_library:
                        expected_users[texture_name] = (
                            expected_users.get(texture_name, 0) + 1
                        )
                ...
                for item in getattr(rig, "fbp_images", ()):
                    try:
                        ...
                        if (
                            bool(getattr(item, "managed_image", False))
                            and bool(image.get(DRAWING_MANAGED_IMAGE_KEY, False))
                        ):
        ...
        pass
    return active, expected_users, candidates


def _drawing_active_image_names():
    active, _expected_users, _candidates = _drawing_buffer_usage_snapshot()
    return active
...
def _image_for_item(item, *, allow_load=None):
    ...
    try:
        ...
        image = None
    expected_path = _normalized_image_path(str(getattr(item, "filepath", "") or ""))
    if image is not None and not _is_compatible_drawing_image(image, expected_path):
        image = None
    if image is not None:
        return image
    ...
    if image is not None:
        try:
            ...
            # Ownership follows the Image datablock tag, never a stale row flag.
            # This prevents cache trimming from freeing a user-owned image after
            # relinking or Undo restores a different datablock into the row.
            item.managed_image = bool(image.get(DRAWING_MANAGED_IMAGE_KEY, False))
    ...
    try:
        ...
        # Never let Blender return a SEQUENCE/MOVIE datablock that happens to
...
def fbp_drawing_render_ready(rig):
    ...
    try:
        ...
        for index in _drawing_required_render_indices(rig):
            ...
            if not packed and (not path or not os.path.isfile(path)):
                return False
        return True
    except (AttributeError, ReferenceError, RuntimeError, TypeError, ValueError, OSError):
        return False
...
def _load_library_image(path, image_index=None):
    ...
    if existing is not None and not _is_compatible_drawing_image(existing, key):
        existing = None
    image = existing or bpy.data.images.load(absolute, check_existing=False)
    if str(getattr(image, "source", "") or "").upper() != "FILE":
        image.source = "FILE"
...
class FBP_OT_MoveDrawing(Operator):
    ...
    direction: StringProperty(description="Requested movement or step direction for this action, such as previous, next, up or down.", default="UP")
...
def draw_drawing_plane_ui(layout, context, rig):
    ...
    previous_row = slider.row(align=True)
    previous_row.enabled = drawing_index > 0
    previous = previous_row.operator("fbp.drawing_index_step", text="", icon="TRIA_LEFT")
    previous.direction = "PREV"
    slider.prop(rig, f'["{DRAWING_INDEX_KEY}"]', text="Drawing", slider=True)
    next_row = slider.row(align=True)
    next_row.enabled = drawing_index < drawing_count
    next_op = next_row.operator("fbp.drawing_index_step", text="", icon="TRIA_RIGHT")
