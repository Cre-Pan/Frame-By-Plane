...
def fbp_order_sequence_files(files, *, reverse=False):
    """Return a stable natural order for one detected image sequence.

    The helper centralizes sequence ordering so every import path agrees on the
    same direction. Videos and one-file layers are intentionally unaffected by
    the reverse option at their call sites.
    """
    ordered = sorted([name for name in files if name], key=natural_sort_key)
    if reverse and len(ordered) > 1:
        ordered.reverse()
    return ordered
...
def fbp_expand_sequences_as_individual_planes(
    ...
    *,
    ...
    """Prepare detected rows for animated or individual-plane import.

    ``reverse_sequences`` changes only rows positively identified as image
    sequences. The exact same ordered list is then used by both animated planes
    and the Individual Planes workflow, preventing the two modes from drifting.
    """
    prepared = []
    for layer_name, files, is_sequence in grouped_layers or ():
        ordered_files = fbp_order_sequence_files(
            files,
...
def fbp_scan_project_layers_for_setup(root, *, separate_sequences=False, reverse_sequences=False):
    ...
    def visit(node, collection_name=""):
        ...
        detected_layers = fbp_group_direct_images_into_layers(imgs, folder_name)
        grouped_layers = fbp_expand_sequences_as_individual_planes(
            detected_layers,
            enabled=separate_sequences,
            reverse_sequences=reverse_sequences,
        )

        is_root = os.path.normcase(os.path.abspath(path)) == root_key
        # Sequence reading changes only how media becomes planes. It must not
        # change the collection hierarchy inferred from the source folders.
        flatten_single_layer = (
            not is_root
            and fbp_should_flatten_leaf_folder(detected_layers, child_nodes)
        )
...
_FBP_FAST_IMPORT_RUNTIME.setdefault("queued_names", [])
_FBP_FAST_IMPORT_RUNTIME.setdefault("queued_name_set", set())
...
def fbp_fast_import_queued_names(context=None):
    """Return the current native-build queue without serializing it per layer."""
    try:
        return list(_FBP_FAST_IMPORT_RUNTIME.get("queued_names", ()))
...
def fbp_set_fast_import_queued_names(names, context=None):
    ...
    for name in tuple(names or ()):
        ...
        if name and name not in seen:
            unique.append(name)
            seen.add(name)
    _FBP_FAST_IMPORT_RUNTIME["queued_names"] = unique
    _FBP_FAST_IMPORT_RUNTIME["queued_name_set"] = seen
    return True


def fbp_queue_fast_import_rig_name(name):
    """Append one generated rig in O(1), preserving stable finalization order."""
    name = str(name or "")
    if not name:
        return False
    try:
        seen = _FBP_FAST_IMPORT_RUNTIME.setdefault("queued_name_set", set())
        if name in seen:
            return False
        _FBP_FAST_IMPORT_RUNTIME.setdefault("queued_names", []).append(name)
        seen.add(name)
        return True
    except (AttributeError, TypeError, ValueError):
        return False
...
def fbp_child_entries(path, *, separate_sequences=False, reverse_sequences=False, content_cache=None):
    entries = []
    if content_cache is None:
        content_cache = {}
    try:
        with os.scandir(path) as scan:
            directory_entries = sorted(scan, key=lambda entry: natural_sort_key(entry.name))
    except (OSError, PermissionError):
        return entries
    ...
    for entry in directory_entries:
        ...
        try:
            if entry.is_dir(follow_symlinks=False):
                if fbp_folder_has_importable_content(entry.path, cache=content_cache):
                    entries.append(('DIR', name, entry.path))
                continue
            if not entry.is_file(follow_symlinks=True):
                continue
        except OSError:
            continue
        if is_supported_media_file(name) and (is_supported_video_file(name) or not is_technical_map_file(name)):
            image_names.append(name)

    # Sort directory entries before appending media rows. A final global sort
    # would silently restore ascending order after a sequence was intentionally
    # reversed and expanded into Individual Planes.
    entries.sort(key=lambda e: natural_sort_key(e[1]))
    for layer_name, files, is_sequence in fbp_expand_sequences_as_individual_planes(
        fbp_group_direct_images_into_layers(image_names),
        enabled=separate_sequences,
        reverse_sequences=reverse_sequences,
    ):
        if is_sequence:
...
def fbp_folder_has_importable_content(path, cache=None):
    """Return whether a folder tree contains importable media.

    ``cache`` is scoped to one import operation. It avoids traversing the same
    Toon Boom/animation subtree repeatedly and ignores directory symlinks so a
    junction or cyclic link cannot recurse forever.
    """
    if cache is None:
        cache = {}
    try:
    ...
    # Mark as false before recursion to break unexpected filesystem cycles.
    cache[key] = False
    try:
        with os.scandir(path) as entries:
            for entry in entries:
                ...
                try:
                    if entry.is_dir(follow_symlinks=False):
                        if fbp_folder_has_importable_content(entry.path, cache=cache):
                            cache[key] = True
                            return True
                        continue
                    if not entry.is_file(follow_symlinks=True):
                    ...
                    continue
                if is_supported_media_file(name) and (
                    is_supported_video_file(name) or not is_technical_map_file(name)
                ):
                    cache[key] = True
                    return True
    except OSError:
...
def fbp_folder_direct_dirs(path, content_cache=None):
    if content_cache is None:
        content_cache = {}
    directories = []
    try:
...
def fbp_collect_mixed_folder_entries(base, *, separate_sequences=False, reverse_sequences=False):
    """Collect top-level folder/image entries for the Pending List."""
    entries = []
    try:
        with os.scandir(base) as scan:
            source_entries = sorted(scan, key=lambda entry: natural_sort_key(entry.name))
    except (OSError, PermissionError):
        return entries
    direct_images = []
    for entry in source_entries:
        ...
        try:
            if entry.is_dir(follow_symlinks=False):
                files = fbp_folder_direct_images(entry.path)
                if files:
                    folder_label = clean_layer_name_from_path(name)
                    for layer_name, grouped_files, is_sequence in fbp_expand_sequences_as_individual_planes(
                        fbp_group_direct_images_into_layers(files, folder_label),
                        enabled=separate_sequences,
                        reverse_sequences=reverse_sequences,
                    ):
                        entries.append((layer_name, entry.path, grouped_files, 'FOLDER_SEQUENCE' if is_sequence else 'FOLDER_IMAGE'))
                continue
            if not entry.is_file(follow_symlinks=True):
                continue
        except OSError:
    ...
    for layer_name, grouped_files, is_sequence in fbp_expand_sequences_as_individual_planes(
        fbp_group_direct_images_into_layers(direct_images),
        enabled=separate_sequences,
        reverse_sequences=reverse_sequences,
    ):
        entries.append((layer_name, base, grouped_files, 'IMAGE_SEQUENCE' if is_sequence else 'IMAGE'))
    # Keep the order emitted by the sequence preparer. Sorting by layer name here
    # would undo Reverse Sequence for Individual Planes.
    return entries


def fbp_build_project_folder(
    context, folder_path, parent_collection, cursor_loc, depth_counter,
    color_seed=0, depth=0, color_state=None, separate_sequences=False,
    reverse_sequences=False, content_cache=None,
):
    ...
    direct_dirs = fbp_folder_direct_dirs(folder_path, content_cache=content_cache)
    detected_layers = fbp_group_direct_images_into_layers(direct_images, folder_name)
    grouped_layers = fbp_expand_sequences_as_individual_planes(
        detected_layers,
        enabled=separate_sequences,
        reverse_sequences=reverse_sequences,
    ...
    # Preserve folder flattening independently from the selected sequence mode.
    flatten_single_layer = fbp_should_flatten_leaf_folder(detected_layers, direct_dirs)
