...
class FBP_OT_CreateCustomNodeEffect(Operator):
    ...
    kind: EnumProperty(description="Choose the custom effect backend. Automatic follows the current Effects view; Shader creates a color/alpha/UV group; Geometry creates a mesh pass-through group.",
        name="Type",
        items=(
    ...
    def execute(self, context):
        ...
        try:
            ...
            changed_rigs = [
                ...
                if _geometry_nodes.fbp_add_effect(
                    rig, effect_id, sync_items=False
                )
            ]
            changed = len(changed_rigs)
            for rig in changed_rigs:
                while _geometry_nodes.fbp_can_move_effect(rig, effect_id, "UP"):
                    if not _geometry_nodes.fbp_move_effect(
                        rig, effect_id, "UP", sync_items=False
                    ):
                        break
            if changed:
                _geometry_nodes.fbp_sync_effect_items(
                    rigs[0], rigs,
                    repair_assets=False,
...
class FBP_OT_RegisterCustomNodeEffect(Operator):
    ...
    effect_icon: EnumProperty(
        name="Icon",
        description="Blender icon displayed beside this custom effect in the Add Effect menu and Effects stack. Changing it affects presentation only and does not rebuild the node group.",
        items=CUSTOM_EFFECT_ICON_ITEMS,
        default="NODETREE",
    )
    description: StringProperty(
        name="Description",
        description="Detailed help text shown when the pointer rests over this custom effect in Frame By Plane menus and the Effects stack.",
