"""Shared effect-definition and instance foundations for Frame by Plane 5.1.x.
...
FBP_EFFECT_SCHEMA_VERSION = 8
FBP_EFFECT_INSTANCE_VERSION = 1
FBP_ALPHA_GEOMETRY_CONTRACT_VERSION = 1
...
def validate_effect_registry(registry):
    ...
    for effect_id, definition in registry.items():
        ...
        if definition.get("camera_aware"):
            if kind != "GEOMETRY":
                issues.append(f"{prefix}: camera-aware effects must be Geometry Nodes effects")
            if not isinstance(camera_contract, dict):
                issues.append(f"{prefix}: camera contract is not a mapping")
            elif not str(camera_contract.get("object_socket", "") or "").strip():
...
def refresh_effect_instance_id(owner, effect_id, owner_token=""):
    """Force a fresh identity, used after duplicating a complete FBP stack."""
    return assign_effect_instance_id(
        owner,
        effect_id,
        new_effect_instance_id(effect_id),
        owner_token,
    )
